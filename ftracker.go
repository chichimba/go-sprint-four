...
// meanSpeed возвращает значение средней скорости движения (км/ч) во время тренировки.
//
// Параметры:
//
// action int — количество совершенных действий(число шагов при ходьбе и беге, либо гребков при плавании).
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	energyRun := ((float64(runningCaloriesMeanSpeedMultiplier) * meanSpeed(action, duration) *
		runningCaloriesMeanSpeedShift) * weight / mInKm * duration * float64(minInH))
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	energyWalk := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed(action, duration)*kmhInMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH))
	return energyWalk

}

// Константы для расчета калорий, расходуеsмых при плавании.
const (
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	energySwim := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimmingCaloriesMeanSpeedShift) *
		float64(swimmingCaloriesWeightMultiplier) * weight * duration
